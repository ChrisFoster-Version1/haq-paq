...
.bench.BenchEachStep:flip`fn`section`datetime!"ssp"$\:();

// Basic upsert which takes a function name(fn) and a section in the function(symbol name) as inputs
.bench.logStep:{[fn;sect]
  `.bench.BenchEachStep upsert (fn;sect;.z.P)
 };

.bench.returnTimeStep:{[fn1;sect1]
  update diff:-':[`time$first datetime;`time$datetime] from select from .bench.BenchEachStep where fn=fn1,sect=sect1
 };
